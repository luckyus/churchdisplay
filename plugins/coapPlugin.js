...
function configure() { //#G
	helper.addDevice(model, 'coapDevice', 'A CoAP Device',
		'A CoAP Device', {
			'co2': {
				'name': 'CO2 Sensor',
	...
	me = model.things.coapDevice.sensors.co2;
	pluginName = model.things.coapDevice.name;
}

exports.start = function(m, params, app) {
	localParams = params;
	model = m;
	configure(app);
...
function simulate() {
	interval = setInterval(function() {
		me.value = helper.randomInt(0, 1000);
		showValue();
	}, localParams.frequency);
